...
예를 들어, "(()))("와 같은 문자열은 "균형잡힌 괄호 문자열" 이지만 
"올바른 괄호 문자열"은 아닙니다.
반면에 "(())()"와 같은 문자열은 "균형잡힌 괄호 문자열" 이면서 동시에 
"올바른 괄호 문자열" 입니다.
...
균형잡힌 괄호 -> 올바른 괄호로 변환
1. 균형잡힌 u, v 문자열로 변환
2. u가 올바른 괄호인 경우 v로 다시 시작
"""


def solution(p: str) -> str:
    def is_balanced(msg: str) -> tuple[list[int], bool]:
        depth = 0
        is_correct = True
        result = []
        for i, m in enumerate(msg):
            ...
            if depth < 0:
                is_correct = False
        return result, depth == 0 and is_correct

    balance_indices, is_correct = is_balanced(p)
    if is_correct:
        return p

    idx = balance_indices[0] + 1
    u = p[:idx]
    v = p[idx:]

    _, is_correct = is_balanced(u)
    if is_correct:
        return u + solution(v)


    new_u = "".join(')' if k == '(' else '(' for k in u[1:-1])
