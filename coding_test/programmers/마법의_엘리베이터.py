...
"""
마법의 세계에 사는 민수는 아주 높은 탑에 살고 있습니다. 탑이 너무 높아서 걸어 다니기 힘든 민수는 마법의 엘리베이터를 만들었습니다. 마법의 엘리베이터의 버튼은 특별합니다. 마법의 엘리베이터에는 -1, +1, -10, +10, -100, +100 등과 같이 절댓값이 10^c (c ≥ 0 인 정수) 형태인 정수들이 적힌 버튼이 있습니다.
...
def solution(storey: int) -> int:
    ...
    queue = deque([(storey_list.copy(), 0)])
    seen = set()

    while queue:
        s_list, n_use = queue.popleft()

        if n_use >= answer:
            continue

        key = tuple(s_list) + (n_use,)
        if key in seen:
            continue
        else:
            seen.add(key)
        ...
        for i in range(len(s_list)):
            ...
            new_n_use = n_use
            new_n_use += new_s_list[i]
            new_s_list[i] = 0
            queue.append((new_s_list, new_n_use))

            new_s_list = s_list.copy()

            new_n_use = n_use
            new_n_use += 10 - new_s_list[i]
            new_s_list[i] = 0
            j = i - 1
            if j < 0:
                new_s_list.insert(0, 1)
...
if __name__ == "__main__":
    tests = [[16], [909], [2554], [19016], [15555], [12_345_678]]
    answers = [6, 4, 16, 9, 19, 22]
