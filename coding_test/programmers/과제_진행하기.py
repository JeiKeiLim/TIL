...
40분 후인 "01:10"에 "math" 과제를 마칩니다. 따라서 "korean", "english", "math" 순으로 과제를 끝내므로 차례대로 배열에 담아 반환합니다.

입출력 예 #2  
"music", "computer", "science", "history" 순으로 과제를 시작합니다.  
시각       진행 중 과제       잠시 멈춘 과제  
"12:20"    "music"           []  
"12:30"    "computer"        ["music"]  
"12:40"    "science"         ["music", "computer"]  
"13:30"    "computer"        ["music"]  
"14:00"    "history"         ["music", "computer"]  
"14:30"    "computer"        ["music"]  
"15:30"    "music"          []  
"16:00"    -                 []  
...
def solution(plans: List[List[str]]) -> List[str]:
    def time_to_mins(t_str: str) -> int:
        hour, min = map(int, t_str.split(":"))
        return hour * 60 + min

    answer = []
    # Or use heapq
    homework_times = [[] for _ in range(24 * 60)]
    for subject, s_time, duration in plans:
        time_int = time_to_mins(s_time)
        homework_times[time_int] = [subject, int(duration)]
    ...
    for i in range(24 * 60):
        if homework_times[i]:
            if current_hw:
                pending_hw.append(current_hw)
            current_hw = homework_times[i]

        if not current_hw and pending_hw:
            current_hw = pending_hw.pop()

        if current_hw:
            current_hw[1] -= 1
            if current_hw[1] <= 0:
                answer.append(current_hw[0])
                current_hw = []
    if current_hw:
        answer.append(current_hw[0])

    answer += [subject for subject, _ in pending_hw[::-1]]
